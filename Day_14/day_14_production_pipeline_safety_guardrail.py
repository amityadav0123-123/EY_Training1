# -*- coding: utf-8 -*-
"""Day_14_Production_pipeline_Safety_Guardrail.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1IerTeg0MlyUNved8aTmC94QAe5M6x3TK
...
lc_retriever   = vectorstore.as_retriever(search_kwargs={'k': 3}) # Renamed to lc_retriever
...
# Ensure SIMULATION_MODE and OPENAI_API_KEY are defined
# if this cell is run before the API Key Configuration cell (`_ZqR24QYVgYf`)
if 'OPENAI_API_KEY' not in globals():
    OPENAI_API_KEY = os.environ.get('OPENAI_API_KEY', '')
if 'SIMULATION_MODE' not in globals():
    SIMULATION_MODE = not bool(OPENAI_API_KEY)
...
class CreditLensPipeline:
    ...
    def run(self, query: str, user_id: str = 'SYSTEM') -> dict:
        ...
        try:
            retrieved_docs = lc_retriever.get_relevant_documents(clean_query) # Use lc_retriever
...
# Ensure accelerate is installed for robust CPU performance with transformers
...
try:
    ...
    for q in comparison_queries:
        # LangChain
        lc_docs = lc_retriever.get_relevant_documents(q) # Use lc_retriever
    ...
    print(f"❌ An error occurred during LlamaIndex setup: {e}")
...
"""### LangGraph Implementation of Human-in-the-Loop Workflow"""

from langgraph.graph import StateGraph, END
from typing import TypedDict

# The LoanRequest and ReviewDecision dataclasses are already defined in the previous cell.
...
# Each function takes the current state and returns a dictionary to update the state.

# Router Node: Decides if human review is needed
def route_loan_decision_node(state: HITLGraphState) -> HITLGraphState:
    request = state['loan_request']
    if (request.loan_amount >= hitl.HIGH_VALUE_THRESHOLD or
            request.risk_score >= hitl.HIGH_RISK_SCORE):
        print(f"  [{request.application_id}] Routing to HUMAN REVIEW (Amount: {request.loan_amount:,.0f}, Risk: {request.risk_score:.2f})")
        return {'route_decision': 'human_review'}
    else:
        print(f"  [{request.application_id}] Routing to AUTO PROCESS (Amount: {request.loan_amount:,.0f}, Risk: {request.risk_score:.2f})")
        return {'route_decision': 'auto_process'}

# Auto-Processing Node: Simulates automatic approval
def auto_process_node(state: HITLGraphState) -> HITLGraphState:
    request = state['loan_request']
    # In a real system, this would trigger an automated approval/processing system.
    print(f"  [{request.application_id}] AUTO-PROCESSED: Loan approved without human intervention.")
    # Update loan_request status here if needed, e.g., request.status = 'approved'
    return state

# Human Review Node: Simulates human decision
def human_review_node_graph(state: HITLGraphState) -> HITLGraphState:
    request = state['loan_request']
    # Simulate human decision (based on the previous logic)
    decision = ReviewDecision.APPROVE if request.risk_score < 0.70 else ReviewDecision.REJECT
    reviewer = f'LANGGRAPH_UNDERWRITER_001'
    notes = 'LangGraph simulated manual review complete'

    # Update the loan request within the state
    request.human_decision = decision
    request.reviewer_id    = reviewer
    request.review_notes   = notes

    print(f"  [{request.application_id}] HUMAN REVIEWED: Decision → {decision.value.upper()} by {reviewer}")
    # In a real system, this would log the human decision and trigger further actions.
    return state

# 3. Define the Conditional Edge Logic
def decide_next_step(state: HITLGraphState):
    if state['route_decision'] == 'human_review':
...
# Add nodes
workflow.add_node("route_loan_decision", route_loan_decision_node)
workflow.add_node("auto_process_node", auto_process_node)
workflow.add_node("human_review_node", human_review_node_graph)

# Set entry point
workflow.set_entry_point("route_loan_decision")
...
workflow.add_edge("auto_process_node", END)
workflow.add_edge("human_review_node", END)

# Compile the graph
app = workflow.compile()

# 5. Test the LangGraph workflow
print("\n--- Running LangGraph Simulation ---")
for loan in test_loans:
    print(f"\nProcessing loan: {loan.application_id}")
    final_state = app.invoke({"loan_request": loan, "route_decision": None})
    processed_loan = final_state['loan_request']
    status = processed_loan.human_decision.value if processed_loan.human_decision else 'Auto-Approved'
    print(f"Final Status for {processed_loan.application_id}: {status}")

"""### Re-compiling and Testing the LangGraph Workflow"""

# 4. Build the LangGraph (re-compiling)
print("\n--- Re-building LangGraph Workflow ---")
workflow = StateGraph(HITLGraphState)
...
# 5. Test the LangGraph workflow (re-running simulation)
print("\n--- Re-running LangGraph Simulation ---")
for loan in test_loans:
    print(f"\nProcessing loan: {loan.application_id}")
    final_state = app.invoke({"loan_request": loan, "route_decision": None})
    processed_loan = final_state['loan_request']
...
#Here we are doing Extension Task 2 Using LangGraph framework
...
# Creating Nodes for agents
# here there are 3 agents so we will create 3 node
...
# Now we will Test this sample data
...
if audit_log:
    ...
    print("   No audit events to display. Run some queries through the pipeline first.")
